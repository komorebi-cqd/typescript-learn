...
## 在node环境中安装ts 建议全局安装（这样会有一个全局的命令tsc）
//全局安装
- npm i -g typescript
...
使用类型别名可以实现类似的组合效果，需要通过```&```，它叫交叉类型，如果有相同的值会合并类型
...
class User{
    ...
    gender: '男'| '女' = '男'
    pid?: string
    constructor(name:string,public age: age,gender:'男'| '女'){ //属性加public就是简写
        this.name = name;
        this.age = age;
    }
}
...
如果某个属性，通过构造函数的参数传递，并且不做任何处理的赋值给给属性可以简写，前面加public
